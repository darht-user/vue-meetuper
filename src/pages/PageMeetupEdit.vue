    ...
    <section class="hero">
      <div class="hero-body">
        ...
        <div class="is-pulled-right">
          <!-- Update Button -->
          <button @click="update" class="button is-success is-large">Update</button>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
      ...
      this.fetchMeetupById(this.meetupId)
        .then(meetupId => {
          if (this.meetupCreator._id !== this.authUser._id) {
            this.$router.push({ path: "not-owner" });
          }
        })
    ...
    update() {
      this.updateMeetup(this.meetup).then(res => {
        this.$toasted.success("Meetup Successfuly Updated!", {
          duration: 3000,
          position: "top-center"
        });
        this.$router.push({ path: `/meetups/${this.meetup._id}` });
      });
    }
  }
};
